...
- The named onboarding coordinate space must wrap both the measured app content and the overlay that consumes the preferences. In SwiftUI modifier chains, place `.coordinateSpace(name:)` outside the `.overlayPreferenceValue` layer when the overlay also reads that named space; otherwise the overlay geometry can inherit a different safe-area origin than the target frames.
- For small controls in macOS titlebar-adjacent chrome, attach the target to the visible control or to a stable wrapper with the same visual bounds. Do not recover missing anchors by guessing absolute offsets from a larger parent frame; fix the target attachment or coordinate-space ownership instead.
